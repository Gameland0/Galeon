...
class MessageProcessingService {
  ...
  async processMessage(userId, message, agentId, conversationId) {
    try {
      ...
      if (agentId) {
        ...
        // systemMessage = `Assistant Role: ${agent.name}
        //   Description: ${agent.description}
        //   Role Type: ${agent.role || "not specified"}
        //   Goal: ${agent.goal || "not specified"}
        //   Knowledge Context: ${relevantKnowledge}
        //   Previous Context: ${previousMessages}
        //   ${relevantCode ? `Code Context: ${relevantCode}` : ""}`;
        context = this.buildModelContext(agent, message, relevantKnowledge, previousMessages, relevantCode);
      } else {
      ...
      if (agent && agent.model) {
        aiResponse = await this.getModelResponse(agent.model, [context],message);
      } else {
        aiResponse = await OpenAIService.createChatCompletion([context,{ role: "user", content: message }]);
      }
      if (agent) {
      ...
      let canUseCredit
      const buyBalance = await CreditService.getBuyCredit(userId)
      if (buyBalance) {
        canUseCredit = await CreditService.useBuyCredit(userId);
      } else {
        canUseCredit = await CreditService.useCredit(userId);
      }
  ...
  // async processMessage(userId, message, agentId, conversationId, chainid) {
  //   try {
  //     const agent = await this.getAgentById(agentId);
  //     if (!agent) {
  //       throw new Error('Agent not found');
  //     }
  //     const enhancedResult = await KnowledgeIntegrationService.enhanceMessageProcessing(
  //       message,
  //       agentId,
  //       conversationId,
  //       {
  //         platform: agent.chainid,
  ...
  //         {
  //           role: "system",
  //           content: this.buildEnhancedPrompt(agent, enhancedResult.context.codeKnowledge)
  //         },
  //         { role: "user", content: enhancedResult.message }
  //       ]
  //     );
  ...
  //       await this.saveNewCodeVersion(
  //         processedResponse.files[0],
  //         latestCode?.version || 0,
  //         conversationId,
  //         agentId
  //       );
  //     }
  ...
  //       agent,
  //       files: processedResponse.files,
  //       codeVersions: enhancedResult.context.codeVersions
  //     };
  //   } catch (error) {
  //     console.error('Error in processMessage:', error);
  //     throw error;
  ...
  async getModelResponse(model, messages, message) {
    // 确保消息格式统一
    ...
    try {
      switch (model) {
        ...
        case "gpt-4o-mini":
          ...
          return await OpenAIService.createChatCompletion(formattedMessages, model);
        ...
        case 'claude-3-haiku-20240307':
          ...
          // otherMsgs[0].content = otherMsgs[0].content + '\n' + systemMsg.content
          return await ClaudeService.createChatCompletion([
            ...messages
          ], model);
          
        case "gemini-pro":
          return await GeminiService.generateContent(formattedMessages);
          
        default:
          return await OpenAIService.createChatCompletion(formattedMessages);
      }
    } catch (error) {
  ...
  async buildEnhancedPrompt(userId ,agent, expertise, relevantCode = '', message) {
    if (!agent) return '';
    const previousMessages = await this.getPreviousMessages(userId, 5);
    const relevantKnowledge = await this.getRelevantKnowledge(
      agent.id,
    ...
    const expertisePrompt = expertise
      ? `\nAgent expertise in:\n${expertise.map(e => `- ${e.concept_type}: ${e.name}`).join('\n')}` 
      : '';

    return `You are ${agent.name}, an AI agent with the following details:
    Role: ${agent.role || 'not specified'}
    Goal: ${agent.goal || 'not specified'}
    Type: ${agent.type || 'not specified'}
    Description: ${agent.description || ''}${expertisePrompt}
    Knowledge Context: ${relevantKnowledge}
    Previous Context: ${previousMessages}
    ${relevantCode ? `Code Context: ${relevantCode}` : ""}
    `;
  ...
  async saveNewCodeVersion(file, previousVersion, conversationId, agentId) {
    await DatabaseService.query(
      `INSERT INTO code_versions (
        id, file_id, version_number, content, 
        agent_id, created_at
      ) VALUES (?, ?, ?, ?, ?, NOW())`,
      [
        UUID.v4(),
  ...
  async compareCode(oldCode, newCode) {
    // 实现简单的代码比较逻辑
  ...
  addChangeSummary(content, differences) {
    const summary = differences.map(diff =>
      `Line ${diff.line}: ${diff.type} - ${diff.content}`
    ).join('\n');
  
    return `${content}\n\nChanges made:\n${summary}`;
  }
  
  buildModelContext(agent, message, relevantKnowledge, previousMessages, relevantCode) {
    if (agent.model?.toLowerCase().includes('claude')) {
      return this.buildClaudeContext(agent, message, relevantKnowledge, previousMessages, relevantCode);
    }
    return this.buildOpenAIContext(agent, message, relevantKnowledge, previousMessages, relevantCode);
  }

  buildClaudeContext(agent, message, relevantKnowledge, previousMessages, relevantCode) {
    const context = `You must respond as ${agent.name}, a specific AI agent with the following traits and knowledge:
  
  Background:
  - You are a specialized ${agent.role} agent, NOT a general AI language model
  - Your specific description: ${agent.description}
  - Your role: ${agent.role}
  - Your goal: ${agent.goal || "to provide expert assistance in my specialized domain"}
  ...
  IMPORTANT: Do not identify yourself as an AI language model or Claude. You must only respond as ${agent.name} with your specific role and expertise as described above. Your responses should reflect your specialized knowledge as a ${agent.role}.
  ...
  buildOpenAIContext(agent, message, relevantKnowledge, previousMessages, relevantCode) {
    return {
      role: 'system',
      content: `You are ${agent.name}, an AI agent with the following description: ${agent.description}. 
      ...
      Remember to maintain your role and expertise as ${agent.role} throughout the response.
      Use your knowledge and experience to provide accurate and helpful answers.
      If code is involved, ensure it follows best practices for ${agent.role} development.`
    };
  }
