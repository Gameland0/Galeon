...
exports.finalizeAgentCreation = async (req, res) => {
  const { address, name, description, type, ipfsHash, role, goal, transactionHash, chainid, imageUrl } = req.body;
  try {
    // 创建agent（后端存储）
...
exports.updateAgent = async (req, res) => {
  const { agentId } = req.params;
  const { name, description, type, model } = req.body;
  try {
    const updatedAgent = await AIService.updateAgent(agentId, req.userId, { name, description, type, model });
...
exports.updateAgentHash = async (req, res) => {
  const { agentId } = req.params;
  const { transactionHash } = req.body;
  try {
    const updatedAgent = await agentService.updateAgentHash(agentId, req.userId, transactionHash);
    res.json(updatedAgent);
  } catch (error) {
    console.error('Error updating agent:', error);
    res.status(500).json({ error: 'Failed to update agent' });
