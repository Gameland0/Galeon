...
class RiskController extends BaseAgent {
  constructor() {
    ...
    this.log('✅ RiskController initialized with on-chain balance support');
  }
  ...
  async checkTradeRisk(strategyOrUserId, signal, tradeAmount) {
    ...
    try {
      // 1. 检查用户是否启用自动交易
      if (!userConfig) {
        const failReason = '用户配置不存在';
        risks.push({ level: 'CRITICAL', reason: failReason });
        this.publish('risk:check_failed', { userId, token: signal.token_symbol, reason: failReason });
        return { passed: false, risks };
      }

      if (!userConfig.enabled) {
        const failReason = '自动交易未启用';
        risks.push({ level: 'CRITICAL', reason: failReason });
        this.publish('risk:check_failed', { userId, token: signal.token_symbol, reason: failReason });
        return { passed: false, risks };
      }
      ...
      // 4.5 🆕 检查是否已持有该代币 (防止重复买入)
      const duplicateCheck = await this.checkDuplicatePosition(userId, signal.token_symbol);
      if (!duplicateCheck.passed) {
        risks.push({ level: 'CRITICAL', reason: duplicateCheck.reason });
        return { passed: false, risks };
      }
      ...
      this.log(`   ✅ 风险检查通过 (${risks.length} 个警告)`);
      this.publish('risk:check_passed', { userId, token: signal.token_symbol, tradeAmount });
      ...
      this.error(`   ❌ 风险检查失败: ${error.message}`);
      return {
        passed: false,
  ...
  checkSignalType(signal) {
    ...
    if (!allowedTypes.includes(signalType)) {
      this.log(`   ❌ 信号类型不支持: ${signalType} (仅支持 LONG/BUY)`);
      return {
        passed: false,
  ...
  async checkFollowStrategy(userConfig, signal) {
    ...
    this.log(`   📊 跟单策略检查: ${strategy}, Token: ${tokenSymbol}, 置信度: ${confidence}%`);
    this.log(`   🔍 DEBUG: whitelisted_tokens type=${typeof userConfig.whitelisted_tokens}, value=${JSON.stringify(userConfig.whitelisted_tokens)}`);
    ...
    if (strategy === 'WHITELIST') {
      ...
      try {
        ...
        this.error(`   ❌ 解析白名单失败:`, e);
        this.error(`   原始值:`, userConfig.whitelisted_tokens);
        return { passed: false, reason: '白名单配置错误' };
      }

      this.log(`   🔍 DEBUG: Parsed whitelist=${JSON.stringify(whitelist)}, length=${whitelist.length}`);

      // 🔧 修复: 空数组表示跟所有代币
      if (whitelist.length === 0) {
        this.log(`   ✅ 白名单为空，跟踪所有代币`);
        return { passed: true };
      }

      const inWhitelist = whitelist.includes(tokenSymbol);
      if (inWhitelist) {
        this.log(`   ✅ ${tokenSymbol} 在白名单中: ${whitelist.join(', ')}`);
        return { passed: true };
      } else {
        return {
          passed: false,
    ...
    if (strategy === 'TOP_SIGNALS') {
      ...
      if (confidence >= minConfidence) {
        this.log(`   ✅ 置信度 ${confidence}% >= ${minConfidence}%`);
        return { passed: true };
      } else {
        return {
          passed: false,
    ...
    if (strategy === 'TWITTER_KOL') {
      ...
      if (ageMinutes > 20) {
        this.log(`   ⏭️ 信号过旧: ${ageMinutes.toFixed(1)} 分钟 (超过20分钟限制)`);
        return {
          passed: false,
      ...
      this.log(`   ✅ 策略: Twitter KOL 跟单 (无置信度限制, 信号年龄: ${ageMinutes.toFixed(1)} 分钟)`);
      return { passed: true };
    }
    ...
    if (strategy === 'FUSION') {
      ...
      if (isTwitterSignal) {
        ...
        if (ageMinutes > 20) {
          this.log(`   ⏭️ Twitter 信号过旧: ${ageMinutes.toFixed(1)} 分钟 (超过20分钟限制)`);
          return {
            passed: false,
        ...
        this.log(`   ✅ 策略: FUSION - Twitter 信号直接通过 (信号年龄: ${ageMinutes.toFixed(1)} 分钟)`);
        return { passed: true };
      } else {
        // Alpha 信号需要检查置信度
        const minConfidence = parseFloat(userConfig.min_confidence || 70);
        if (confidence >= minConfidence) {
          this.log(`   ✅ 策略: FUSION - Alpha 信号置信度 ${confidence}% >= ${minConfidence}%`);
          return { passed: true };
        } else {
          return {
            passed: false,
    ...
    if (strategy === 'TELEGRAM') {
      ...
      if (ageMinutes > 20) {
        this.log(`   ⏭️ Telegram 信号过旧: ${ageMinutes.toFixed(1)} 分钟 (超过20分钟限制)`);
        return {
          passed: false,
      ...
      this.log(`   ✅ 策略: Telegram 群组跟单 (无置信度限制, 信号年龄: ${ageMinutes.toFixed(1)} 分钟)`);
      return { passed: true };
    }
    ...
    if (strategy === 'MEME') {
      ...
      if (ageMinutes > 20) {
        ...
        this.log(`   ⏭️ ${source} 信号过旧: ${ageMinutes.toFixed(1)} 分钟 (超过20分钟限制)`);
        return {
          passed: false,
      ...
      const source = isTelegramSignal ? 'Telegram' : 'Twitter';
      this.log(`   ✅ 策略: MEME Coin 跟单 (${source}, 信任 KOL, 信号年龄: ${ageMinutes.toFixed(1)} 分钟)`);
      return { passed: true };
    }
    ...
    if (strategy === 'RANGE') {
      ...
      // 检查信号年龄 (拒绝超过4小时的旧信号)
      this.log(`   🔍 DEBUG created_at: value=${signal.created_at}, type=${typeof signal.created_at}, isDate=${signal.created_at instanceof Date}`);

      // 确保 created_at 存在
      if (!signal.created_at) {
        this.log(`   ⚠️ Signal missing created_at field, using current time`);
        this.log(`   ✅ 策略: Range Trading (低买高卖, 信号年龄: 0.0 分钟)`);
        return { passed: true };
      }
      ...
      this.log(`   🔍 DEBUG: signalCreatedAt=${signalCreatedAt}, now=${now}, ageMinutes=${ageMinutes}`);

      // 检查计算结果是否有效
      if (isNaN(ageMinutes)) {
        this.log(`   ⚠️ Failed to calculate signal age, invalid date: ${signal.created_at}`);
        this.log(`   ✅ 策略: Range Trading (低买高卖, 信号年龄: unknown)`);
        return { passed: true };
      }

      if (ageMinutes > 240) { // 4 小时
        this.log(`   ⏭️ Range 信号过旧: ${ageMinutes.toFixed(1)} 分钟 (超过4小时限制)`);
        return {
          passed: false,
      ...
      this.log(`   ✅ 策略: Range Trading (低买高卖, 信号年龄: ${ageMinutes.toFixed(1)} 分钟)`);
      return { passed: true };
    }
      ...
      for (const usdtAddress of usdtAddresses) {
        try {
          ...
          if (balanceFormatted > 0) {
            this.log(`   💰 稳定币 ${usdtAddress.substring(0, 10)}...: ${balanceFormatted} (${decimals}位小数)`);
            totalBalance += balanceFormatted;
          }
        } catch (err) {
          this.error(`   ⚠️ 查询合约 ${usdtAddress} 失败: ${err.message}`);
        }
      }

      this.log(`   💵 稳定币总余额: ${totalBalance}`);

      return totalBalance;

    } catch (error) {
      this.error(`   ❌ 查询 USDT 余额失败: ${error.message}`);
      throw error;
    }
  }
  ...
  async getGasBalanceOnChain(walletAddress, chain) {
    try {
      ...
      this.error(`   ❌ 查询 Gas 余额失败: ${error.message}`);
      throw error;
    }
  }
   ...
   * @returns {Promise<number>} SOL 余额
   */
  async getSolanaBalanceOnChain(walletAddress) {
    try {
      const { Connection, PublicKey, LAMPORTS_PER_SOL } = require('@solana/web3.js');
      const rpcUrl = process.env.QUICKNODE_SOLANA_RPC || 'https://solana-rpc.publicnode.com';
      const connection = new Connection(rpcUrl, 'confirmed');
      const publicKey = new PublicKey(walletAddress);
      const lamports = await connection.getBalance(publicKey);
      return lamports / LAMPORTS_PER_SOL;
    } catch (error) {
      this.error(`   ❌ 查询 Solana 余额失败: ${error.message}`);
      throw error;
    }
  }

  /**
   * 检查余额 (🆕 P1.1: 实时链上查询, 🟣 支持 Solana)
   */
  async checkBalance(userId, tradeAmount, chain) {
    try {
      const userConfig = await this.getUserConfig(userId);

      // 🟣 Solana 余额检查 (SOL 即是交易币种也是 Gas)
      if (chain === 'SOLANA' || chain === 'Solana') {
        const solanaAddress = userConfig.solana_wallet_address || userConfig.wallet_address;

        if (!solanaAddress) {
          return { passed: false, reason: 'Solana 钱包地址未配置' };
        }

        this.log(`   🔍 实时查询 Solana 余额: ${solanaAddress.substring(0, 10)}...`);
        const solBalance = await this.getSolanaBalanceOnChain(solanaAddress);

        this.log(`   💰 SOL 余额: ${solBalance.toFixed(4)} SOL`);
        this.log(`   📊 所需: ${tradeAmount} SOL (交易) + ~0.005 SOL (Gas)`);

        // SOL 需要交易金额 + Gas 费
        const totalNeeded = tradeAmount + 0.005;
        if (solBalance < totalNeeded) {
          return {
            passed: false,
            reason: `SOL 余额不足: 可用 ${solBalance.toFixed(4)} SOL, 需要 ${totalNeeded.toFixed(4)} SOL`
          };
        }
      ...
      this.log(`   🔍 实时查询链上余额: ${walletAddress.substring(0, 10)}... (${chain})`);
      ...
      this.log(`   💰 USDT 余额: $${usdtBalance.toFixed(2)}`);
      this.log(`   ⛽ Gas 余额: ${gasBalance.toFixed(6)} ${chain === 'BSC' ? 'BNB' : 'ETH'}`);

      this.log(`   📊 所需余额: USDT $${tradeAmount.toFixed(2)} (交易金额), Gas由BNB支付`);
      ...
      this.error(`   ❌ 余额检查失败: ${error.message}`);
  ...
  async checkDuplicatePosition(userId, tokenSymbol) {
    try {
      ...
      if (existing.length > 0) {
        this.log(`   ⏭️ 已持有 ${tokenSymbol}, 拒绝重复买入`);
        return {
          passed: false,
  ...
  async checkPositionLimits(userId, userConfig) {
    try {
      ...
      this.log(`   📊 当前持仓: ${positionCount}/${maxPositions}`);
  ...
  async checkDailyLoss(userId, userConfig) {
    try {
      ...
      this.log(`   📉 今日盈亏: ${dailyPnL > 0 ? '+' : ''}$${dailyPnL.toFixed(2)} / $${totalEntryAmount.toFixed(2)} = ${dailyPnLPercent > 0 ? '+' : ''}${dailyPnLPercent.toFixed(2)}%`);

      if (dailyPnLPercent <= lossLimit) {
        // 检查熔断是否启用 (默认启用)
        if (userConfig?.circuit_breaker_enabled === 0) {
          this.log(`   ℹ️ 用户已禁用熔断机制，仅记录亏损`);
        } else {
          ...
          if (!isPaused) {
            // 仅在未暂停时触发熔断
            await this.triggerCircuitBreaker(userId, `Daily loss limit reached: ${dailyPnLPercent.toFixed(2)}%`, userConfig);
            this.log(`   ⚠️ 触发熔断: 单日亏损 ${dailyPnLPercent.toFixed(2)}% (亏损 $${dailyPnL.toFixed(2)} / 入场 $${totalEntryAmount.toFixed(2)})`);
          } else {
            this.log(`   ⏸️ 已处于暂停状态，跳过重复熔断`);
          }
        }
  ...
  async checkWhitelist(tokenSymbol, chain, userConfig) {
    try {
      ...
      // 3. 检查系统白名单 (流动性白名单)
      // 🧪 [测试模式] 临时跳过流动性白名单检查
      this.log(`   ⚠️ [测试模式] 跳过流动性白名单检查: ${tokenSymbol}`);
  ...
  async checkLiquidity(tokenSymbol, chain, userConfig) {
    try {
      ...
      this.log(`   ✅ 流动性充足: $${liquidity.tvl.toLocaleString()}`);
  ...
  async checkConcentration(userId, tokenSymbol, tradeAmount, userConfig) {
    try {
      ...
      this.log(`   📌 ${tokenSymbol} 仓位: ${totalPercent.toFixed(2)}% (限制 ${maxPercent}%)`);
   ...
   * @param {Object} userConfig - 用户配置 (可选，用于获取自定义熔断时长)
   */
  async triggerCircuitBreaker(userId, reason, userConfig = null) {
    try {
      // 使用用户配置的熔断时长，否则使用默认值
      const duration = userConfig?.circuit_breaker_duration || this.defaults.circuitBreakerDuration;
      const pausedUntil = new Date(Date.now() + duration);
      ...
      this.log(`   ⚠️ 熔断触发: ${userId} 暂停至 ${pausedUntil.toLocaleTimeString()}`);
      this.log(`   原因: ${reason}`);
      this.publish('risk:circuit_breaker', { userId, reason, pausedUntil: pausedUntil.toISOString() });

      // 发送通知给用户
      await this.notifyCircuitBreaker(userId, reason, pausedUntil);

    } catch (error) {
      this.error(`   ❌ 熔断失败: ${error.message}`);
    }
  }
   ...
   * 🆕 发送熔断通知给用户
   */
  async notifyCircuitBreaker(userId, reason, pausedUntil) {
    try {
      await DatabaseService.query(`
        INSERT INTO user_notifications (user_id, type, title, message, created_at)
        VALUES (?, 'CIRCUIT_BREAKER', '交易已暂停', ?, NOW())
      `, [userId, `熔断触发: ${reason}。将于 ${pausedUntil.toLocaleString()} 自动恢复。`]);

      this.log(`   📢 已发送熔断通知给用户: ${userId}`);
    } catch (error) {
      this.error(`   ❌ 发送熔断通知失败: ${error.message}`);
    }
  }
   ...
   */
  async checkConsecutiveFailures(userId, userConfig = null) {
    try {
      // 检查熔断是否启用 (默认启用)
      if (userConfig?.circuit_breaker_enabled === 0) {
        this.log(`   ℹ️ 用户已禁用熔断机制`);
      ...
      this.log(`   📊 连续止损: ${consecutiveLosses} 次`);

      // 使用用户配置的阈值，否则使用默认值
      const threshold = userConfig?.circuit_breaker_threshold || this.defaults.circuitBreakerThreshold;
      ...
      this.error(`   ❌ 失败检查错误: ${error.message}`);
      return { triggered: false, count: 0 };
    }
  }
  ...
  async unpauseUser(userId) {
    try {
      ...
      this.log(`   ✅ 已解除暂停: ${userId}`);

      return { success: true };

    } catch (error) {
      this.error(`   ❌ 解除暂停失败: ${error.message}`);
  ...
  async getEnabledUsers(signalId, signalStrategyId = null) {
    try {
      ...
      if (signalStrategyId) {
        this.log(`   📊 信号指定策略ID: ${signalStrategyId}`);

        // 🔧 2026-02-06 修复: 新增 solana_wallet_address 字段, 备份: 原查询无此字段
        ...
        this.log(`   ✅ 找到 ${strategies.length} 个匹配的启用策略 (策略ID: ${signalStrategyId})`);
        return strategies;
      }
      ...
      // 🔧 2026-02-06 修复: 新增 solana_wallet_address 字段
      const strategies = await DatabaseService.query(`
        SELECT
          c.strategy_id,
          c.user_id,
      ...
      this.log(`   📊 信号来源: ${signalSource} (${signalId || 'N/A'})`);
      this.log(`   ✅ 找到 ${strategies.length} 个匹配的启用策略 (涉及 ${uniqueUsers.length} 个用户)`);

      // 🔍 DEBUG: 输出每个策略的信息 (仅调试时开启)
      // strategies.forEach(s => {
      //   this.log(`   🔍 [策略] ID=${s.strategy_id}, 名称=${s.strategy_name}, 类型=${s.follow_strategy}, 用户=${s.user_id.substring(0, 20)}...`);
      // });

      return strategies;

    } catch (error) {
      this.error(`   ❌ 查询策略失败: ${error.message}`);
      return [];
    }
  }
   ...
   * @param {string} userId - 用户ID
   */
  async getCircuitBreakerStatus(userId) {
    try {
      ...
      return {
        success: true,
        data: {
          enabled: c.circuit_breaker_enabled !== 0,
          threshold: c.circuit_breaker_threshold || this.defaults.circuitBreakerThreshold,
          durationMinutes: (c.circuit_breaker_duration || this.defaults.circuitBreakerDuration) / 60000,
          dailyLossLimit: c.daily_loss_limit || this.defaults.dailyLossLimit,
          isPaused: isPaused,
          pausedUntil: c.paused_until,
          pauseReason: c.pause_reason
        }
      };
    } catch (error) {
      this.error(`   ❌ 获取熔断状态失败: ${error.message}`);
      return { success: false, error: error.message };
    }
  }
   ...
   * 🆕 更新用户熔断配置
   * @param {string} userId - 用户ID
   * @param {Object} config - 熔断配置
   */
  async updateCircuitBreakerConfig(userId, config) {
    try {
      const { enabled, threshold, durationMinutes, dailyLossLimit } = config;

      // 验证参数
      if (threshold !== undefined && (threshold < 1 || threshold > 10)) {
        return { success: false, error: '阈值必须在 1-10 之间' };
      }
      if (durationMinutes !== undefined && (durationMinutes < 5 || durationMinutes > 1440)) {
        return { success: false, error: '熔断时长必须在 5-1440 分钟之间' };
      }
      if (dailyLossLimit !== undefined && (dailyLossLimit > 0 || dailyLossLimit < -50)) {
        return { success: false, error: '单日亏损限额必须在 -50% 到 0% 之间' };
      }
      ...
      if (enabled !== undefined) {
        updates.push('circuit_breaker_enabled = ?');
        values.push(enabled ? 1 : 0);
      }
      if (threshold !== undefined) {
        updates.push('circuit_breaker_threshold = ?');
        values.push(threshold);
      }
      if (durationMinutes !== undefined) {
        updates.push('circuit_breaker_duration = ?');
        values.push(durationMinutes * 60000);
      }
      if (dailyLossLimit !== undefined) {
        updates.push('daily_loss_limit = ?');
        values.push(dailyLossLimit);
      }

      if (updates.length === 0) {
        return { success: false, error: '没有要更新的配置' };
      }
      ...
      this.log(`   ✅ 已更新用户 ${userId} 的熔断配置`);

      return { success: true };
    } catch (error) {
      this.error(`   ❌ 更新熔断配置失败: ${error.message}`);
      return { success: false, error: error.message };
    }
  }
}
