...
class DynamicStopLoss extends BaseAgent {
  constructor() {
    ...
    this.config = {
      ...
      // 移动止损配置（默认值，可被 position 级别的配置覆盖）
      trailingActivationPct: 5,   // 盈利 5% 后激活移动止损（默认回退值）
      trailingStopPct: 3,         // 追踪止损比例 3%（默认回退值）
    ...
    this.log('✅ DynamicStopLoss initialized');
  }
  ...
  calculateATR(klines, period = 14) {
    if (!klines || klines.length < period + 1) {
      this.log(`   ⚠️ [ATR] K线数据不足 (${klines?.length || 0}/${period + 1})`);
      return null;
    }
  ...
  getInitialStopLoss(entryPrice, klines, options = {}) {
    ...
    this.log(`   📊 [DynamicSL] 用户止损模式: ${userStopLossMode}`);
    ...
    if (useATR) {
      ...
      this.log(`   📊 [DynamicSL] ATR止损: ATR=${atr.toFixed(6)}, SL=${stopLossPct.toFixed(2)}%, TP=${takeProfitPct.toFixed(2)}%`);
      ...
      result.trailingActivated = false;  // 🔧 修复: 建仓时不激活，等盈利达到 activationPct 后由 updateTrailingStop 激活

      this.log(`   📊 [DynamicSL] Trailing止损: 初始SL=${fallbackSL}%, TP=${fallbackTP}% (等待盈利达阈值后激活追踪)`);
      ...
      if (userStopLossMode === 'ATR') {
        this.log(`   📊 [DynamicSL] 固定止损: SL=${fallbackSL}%, TP=${fallbackTP}% (ATR数据不足,降级为FIXED)`);
      } else {
        this.log(`   📊 [DynamicSL] 固定止损: SL=${fallbackSL}%, TP=${fallbackTP}%`);
      }
    }
  ...
  updateTrailingStop(position, currentPrice) {
    ...
    // 🔧 从 position 读取用户配置的激活阈值和追踪距离，回退到全局默认值
    const activationPct = parseFloat(position.trailing_activation_pct) || this.config.trailingActivationPct;
    const trailingPct = parseFloat(position.trailing_stop_pct) || this.config.trailingStopPct;
    ...
    if (initialStopLossType === 'TRAILING' && !trailingActivated && profitPct >= activationPct) {
      this.log(`   🔄 [Trailing] 激活移动止损: 盈利 ${profitPct.toFixed(2)}% >= ${activationPct}%`);
    ...
    if (result.trailingActivated) {
      // 新止损价 = 最高价 × (1 - 追踪比例)
      const newTrailingStop = result.newHighestPrice * (1 - trailingPct / 100);

      // 止损只能上移，不能下移
      if (newTrailingStop > result.newStopLossPrice) {
        this.log(`   📈 [Trailing] 止损上调: $${result.newStopLossPrice.toFixed(6)} → $${newTrailingStop.toFixed(6)}`);
