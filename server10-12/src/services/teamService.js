...
class TeamService {
  ...
  async processTeamMessage(userId, message, teamId, conversationId) {
    ...
    const context = await this.getConversationHistory(conversationId);
    ...
    try {
      ...
      const processedResponse = await LangChainService.processMessage(message, team, context, combinedKnowledge, blockchainPlatform);
      await this.storeMessage(userId, conversationId, 'system', processedResponse.response);

      if (processedResponse.needsDecomposition) {
        const results = await this.executeDecomposedTasks(processedResponse.taskDecomposition, team, context, conversationId,blockchainPlatform,gameType);
        ...
        await this.updateAgentKnowledge(team, message, processedResponse.response);
        await this.storeMessage(userId, conversationId, 'system', processedResponse.response);
        return {
  ...
  // async generateCode(developer, task, context) {
  //   const prompt = `As a ${developer.role}, generate code for the following task: ${task}. 
  //                   Please provide detailed, well-commented code that addresses the specific requirements.`;
  ...
  async executeDecomposedTasks(taskDecomposition, team, context, conversationId,blockchainPlatform,gameType) {
    const results = [];
    for (const task of taskDecomposition) {
      const executingAgent = team.agents.find(a => a.id === task.assignedAgentId);
      if (executingAgent) {
        try {
          const { originalCode, review, updatedCode } = await this.processSubtask(task, context, blockchainPlatform,gameType);
          ...
          await DatabaseService.saveConversation(null, conversationId, executingAgent.id, 'agent', JSON.stringify({
            subtask: task.description,
            originalCode,
            review,
            updatedCode,
