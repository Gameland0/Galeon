...
class AgentService {
  ...
  async createAgent(userId, agentData) {
    const { name, description, type, role, goal, ipfsHash, transactionHash } = agentData;
    const query = `
      INSERT INTO agents (name, description, type, role, goal, ipfs_hash, transaction_hash, owner)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const result = await DatabaseService.query(query, [name, description, type, role, goal, ipfsHash, transactionHash, userId]);
  ...
  async updateAgent(agentId, userId, { name, description, type, role }) {
    return DatabaseService.query(
      'UPDATE agents SET name = ?, description = ?, type = ?, role = ? WHERE id = ? AND owner = ?',
      [name, description, type, role, agentId, userId]
    );
  }
  ...
  async processTrainingContent(agentId, content) {
    const processedContent = await OpenAIService.processContent(content);
    await this.updateAgentKnowledge(agentId, processedContent);
  }
