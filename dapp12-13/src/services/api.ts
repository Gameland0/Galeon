...
// const API_URL = process.env.NODE_ENV === 'production' ? 'https://testaiservice.gameland.network/api' : 'http://localhost:5000/api'
export const api = axios.create({
  baseURL: API_URL,
  // baseURL:'https://testnet-api.gameland.network'
});
...
export const updataUserCredit = async (data: any) => {
  const response = await api.put('/credits', data);
  return response.data;
};